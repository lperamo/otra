...
$verbose = $argv[1];
$route = $argv[2];
define('BASE_PATH', substr(str_replace('\\', '/', __DIR__), 0, strlen(__DIR__) - strlen('lib/otra/console/deployment/genBootstrap'))); // Fixes windows awful __DIR__, BASE_PATH ends with /.
...
set_error_handler(function ($errno, $message, $file, $line, $context) {
  throw new \lib\otra\OtraException($message, $errno, $file, $line, $context);
});
