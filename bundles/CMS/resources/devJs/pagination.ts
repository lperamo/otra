let p = (function() : any {
  ...
  function entry(e : MouseEvent) : void
  {
    if (13 === e.which)
    {
      let val = $(this).val();
  ...
  {
    let limit = parseInt(p.limitSel.val());
  ...
  function previousPage() : void
  {
    ...
    let oldActive = p.pageLis.filter('.active'),
    ...
    let $$paginationCenter = $('.paginationCenter'),
  ...
  function nextPage() : void
  {
    ...
    let newActive = p.pageLis.filter('.active').removeClass('active').next().addClass('active'),
    ...
    let max = p.offset + limit;
    p.indications.text(p.offset + ' - ' + ((max > count) ? count : max) + ' / ' + count);

    p.dots.toggleClass('hiddenImp', newActive.index() + 1 > p.nbPages - p.nbPagesVisibles);
  }

  function changeLimit(changeTrs) : void
  {
    let limit = parseInt(p.limitSel.val()),
      lis = '';
    ...
    if (p.overflowCont.hasClass('paginationFixedWidth') && 6 > p.nbPages)
      ...
      p.overflowCont.removeClass('paginationFlexWidth').addClass('paginationFixedWidth')
        ...
        .end().nextAll('ul')
        .prepend('<li id="dots"><a>...</a></li><li id="saut"><input id="sautInput" placeholder="#" /></li>');

    for (let i = 1, nbPages = p.nbPages + 1; i < nbPages; i++)
    {
  ...
  function page() : void
  {
    ...
    // On bouge les cases pour mettre les cases les plus proches près de la page actuelle
    let paginationCenterSel = $('.paginationCenter'),
      limiteDroite = Math.ceil(count / limit) - p.nbPagesVisibles;
