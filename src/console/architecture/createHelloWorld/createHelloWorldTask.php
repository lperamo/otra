...
namespace otra\console\architecture\createHelloWorld
{
  ...
  use const otra\config\{BASE_PATH, BUNDLES_PATH, CONSOLE_PATH, CORE_PATH, DIR_SEPARATOR};
  use const otra\console\
    {CLI_BASE,CLI_ERROR,CLI_INFO,CLI_INFO_HIGHLIGHT,CLI_SUCCESS,CLI_TABLE,CLI_WARNING,ERASE_SEQUENCE,END_COLOR};
  use const otra\console\architecture\constants\{ARG_BUNDLE_NAME, ARG_CONTROLLER_NAME, ARG_MODULE_NAME, ARG_INTERACTIVE};
  ...
  define('otra\console\architecture\createHelloWorld\ACTION_NAME', $argv[ARG_ACTION_NAME]);

  if (!defined('otra\console\architecture\createHelloWorld\OTRA_SUCCESS'))
    define('otra\console\architecture\createHelloWorld\OTRA_SUCCESS', CLI_SUCCESS . ' ✔' . END_COLOR . PHP_EOL);
  ...
  define('otra\console\architecture\createHelloWorld\BASE_PATH_LENGTH', strlen(BASE_PATH));
  ...
  function copyAndShow(string $source, string $destination, string $message) : void
  {
    ...
    echo $message, CLI_TABLE,  'BASE_PATH + ', CLI_INFO_HIGHLIGHT, substr($destination, BASE_PATH_LENGTH), END_COLOR,
      ' created', OTRA_SUCCESS;
  }

  // We fixes the action file generated by filling the method (but here we just replaced the whole file)
  define('otra\console\architecture\createHelloWorld\BUNDLE_FOLDER', BUNDLES_PATH . $bundleName . DIR_SEPARATOR);
  define('otra\console\architecture\createHelloWorld\MODULE_FOLDER', BUNDLE_FOLDER . $moduleName . DIR_SEPARATOR);
  $starterAction = MODULE_FOLDER . 'controllers/' . $argv[ARG_CONTROLLER_NAME] . DIR_SEPARATOR . ucfirst(ACTION_NAME) . 'Action.php';
  ...
  */
  if (!defined('otra\console\architecture\createHelloWorld\VERBOSE'))
    define('otra\console\architecture\createHelloWorld\VERBOSE', 0);
